...
const EventList = () => {
  ...
  const { events, fetchEvents, fetchOrganizerEvents,loading, error} = useEventStore();
  const {user : authUser, getMe} = useAuthStore();
    ...
    const loadUser = async () => {
      try {
        ...
        message.error(err?.response?.data?.message || 'Failed to load user info');
      }
    };
    loadUser();
  }, []);
