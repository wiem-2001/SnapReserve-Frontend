...
function CreateEvent() {
  ...
  const validateField = (name, value) => {
    ...
    switch (name) {
      ...
      case 'description':
        ...
        break;
      case 'maxCapacity':
        newErrors.maxCapacity = value && value > 0 ? '' : 'Must be a positive number';
        break;
      default:
        break;
    }
  ...
  const handlePricingChange = (index, key, value) => {
    ...
    const newErrors = { ...errors };
    const hasInvalidPricing = newPricing.some(p => !p.name.trim() || p.price === '' || p.price < 0);
    newErrors.pricingTiers = hasInvalidPricing ? 'All pricing tiers must be valid' : '';
    setErrors(newErrors);
  };
