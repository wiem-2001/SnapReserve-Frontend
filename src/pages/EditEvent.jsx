...
function EditEvent() {
  ...
  useEffect(() => {
    if (event) {
      setFormData({
        ...
        pricingTiers: event.pricingTiers || [{ name: '', price: '' }]
      });
    }
  }, [event]);
  ...
  const addPricingTier = () => {
    setFormData(prev => ({ ...prev, pricingTiers: [...prev.pricingTiers, { ageGroup: '', price: '' }] }));
  };
  ...
  const validate = () => {
    ...
    if (!formData.maxCapacity || formData.maxCapacity <= 0) newErrors.maxCapacity = 'Max capacity must be positive';
    ...
    if (!formData.pricingTiers.length) {
      ...
      formData.pricingTiers.forEach(({ name, price }, i) => {
        if (!name.trim()) newErrors.pricingTiers = `Age group is required at row ${i + 1}`;
        ...
        <Layout.Content style={{ padding: '0px 24px', background: '#fff' }}>
         <div
          ...
          <Form layout="vertical" onFinish={handleSubmit} autoComplete="off">
            ...
            <Form.Item label="Maximum Capacity" validateStatus={errors.maxCapacity ? 'error' : ''} help={errors.maxCapacity} className="custom-input">
              <Input
                type="number"
                name="maxCapacity"
